...
import { getWebsites, deleteWebsite, triggerAudit, getSession } from '../services/websites'
              ...
              >
                <td className="px-3 py-3" onClick={(e) => e.stopPropagation()}>
                  <div className="flex items-center gap-3">
                    <div
                      className="w-14 h-9 rounded-md bg-muted overflow-hidden shrink-0 flex items-center justify-center"
                      style={{ viewTransitionName: `website-og-${ws.id}` }}
                    >
                      {ws.ogImageUrl ? (
                        <img src={ws.ogImageUrl} alt="" className="w-full h-full object-cover" />
                      ) : ws.faviconUrl ? (
                        <img
                          src={ws.faviconUrl}
                          alt=""
                        ...
                        <span className="text-xs text-muted-foreground/60 font-bold select-none">
                    ...
                    <div className="flex flex-col gap-0.5 min-w-0">
                      ...
                      {ws.cronExpression && (
                        <span className="text-[10px] text-muted-foreground/80 leading-tight">
                          {describeCron(ws.cronExpression)}
                        </span>
                      )}
                    </div>
                  </div>
