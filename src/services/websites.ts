...
export const addWebsite = createServerFn({ method: 'POST' })
  .inputValidator((data: { name: string; url: string; formFactor: 'mobile' | 'desktop' }) => data)
  .handler(async ({ data }) => {
    const headers = await getRequestHeaders()
    const session = await auth.api.getSession({ headers })
...
export const updateWebsite = createServerFn({ method: 'POST' })
  .inputValidator((data: { websiteId: string; name: string; url: string; faviconUrl: string | null; ogImageUrl: string | null; formFactor: 'mobile' | 'desktop' }) => data)
  .handler(async ({ data }) => {
    const headers = await getRequestHeaders()
    const session = await auth.api.getSession({ headers })
    ...
    await db.update(website)
      .set({ name: data.name, url: data.url, faviconUrl: data.faviconUrl, ogImageUrl: data.ogImageUrl, formFactor: data.formFactor })
      .where(and(eq(website.id, data.websiteId), eq(website.userId, session.user.id)))
  })
